...
def detect(model, img0):
    ...
    if pred is not None:
        ...
        pred[:, 5:13] = scale_coords_landmarks(img.shape[2:], pred[:, 5:13], img0.shape).round()
        for j in range(pred.size()[0]):
            ...
            #landmarks = (pred[j, 5:13].view(1, 8) / gn_lks).view(-1).tolist()
            #class_num = pred[j, 15].cpu().numpy()
            #class_num = pred[j, 13].cpu().numpy()
            x1 = int(xywh[0] * w - 0.5 * xywh[2] * w)
            y1 = int(xywh[1] * h - 0.5 * xywh[3] * h)
            x2 = int(xywh[0] * w + 0.5 * xywh[2] * w)
            y2 = int(xywh[1] * h + 0.5 * xywh[3] * h)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='weights/plate_detect.pt',
    ...
    parser.add_argument('--dataset_folder', default='D:/car_det/val_detect/blue/', type=str, help='dataset path')
    #parser.add_argument('--folder_pict', default='/yolov5-face/data/widerface/val/wider_val.txt', type=str, help='folder_pict')
    parser.add_argument('--folder_pict', default='D:/car_det/val_detect/blue/ml.txt', type=str, help='folder_pict')
